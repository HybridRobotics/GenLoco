...
"""Gazebo robot interface of A1 robot to test the policy"""
...
from motion_imitation.real_a1.udp_interface import a1_interface
...
class A1Robot(a1.A1):
  ...
  def __init__(self, pybullet_client,
    ...
    # self._velocity_estimator = Gazebo_VelocityEstimator(self)

    # Initiate UDP for robot state and actions : currently hard code
    self._robot_interface = a1_interface(
                                          recv_IP="127.0.0.1",
                                          recv_port=8000,
                                          send_IP="127.0.0.1",
                                          send_port=8001
    ...
    # self._robot_interface = a1_interface()
    # self._robot_interface.send_command(self.GetInitMotorAngles())
    kwargs['on_rack'] = False
  ...
  def ReceiveObservation(self):
    ...
    # print("real_xyzw",self.GetBaseOrientation())
    self._rpyrate = np.array(self._raw_state[4:7])
    # print("rpy_rate", self.GetTrueBaseRollPitchYawRate())
    self._motor_angles = np.array(self._raw_state[7:19])
    self._is_walking = self._raw_state[19]
    # print("_motor_angles", self.GetMotorAngles())
    if self._is_walking:
  ...
  def ApplyAction(self, motor_commands, motor_control_mode=None):
    ...
    if self.walking_count <150:
      motor_commands = self._ClipMotorCommands(motor_commands,0.1)
    else:
      motor_commands = self._ClipMotorCommands(motor_commands,MAX_MOTOR_ANGLE_CHANGE_PER_STEP)
  ...
  def _ClipMotorCommands(self, motor_commands,max_angle=MAX_MOTOR_ANGLE_CHANGE_PER_STEP):
